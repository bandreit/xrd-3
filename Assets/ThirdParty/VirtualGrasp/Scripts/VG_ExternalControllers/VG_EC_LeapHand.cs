...
public class VG_EC_LeapHand : VG_ExternalController
{
    ...
    private Matrix4x4 modifyPose(Matrix4x4 rawPose, bool isWrist)
    {
        /*
        // MegaPack
        return Matrix4x4.TRS(rawPose.GetColumn(3), Quaternion.LookRotation(
            m_handType == VG_HandSide.LEFT ? rawPose.GetColumn(0) : -rawPose.GetColumn(0),
            m_handType == VG_HandSide.LEFT ? rawPose.GetColumn(1) :  rawPose.GetColumn(1)), Vector3.one);
        */
        
		// GleechiRig
        if (isWrist) return Matrix4x4.TRS(rawPose.GetColumn(3), Quaternion.LookRotation(
            -rawPose.GetColumn(1), rawPose.GetColumn(2)), Vector3.one);
        else return Matrix4x4.TRS(rawPose.GetColumn(3), Quaternion.LookRotation(
            m_handType == VG_HandSide.LEFT ? rawPose.GetColumn(0) : -rawPose.GetColumn(0), rawPose.GetColumn(2)),
            Vector3.one);
    }
